...
TableOfContent::TableOfContent(QObject *parent) : QAbstractListModel(parent) {
  ...
  server->route("/(.*)", std::function<QByteArray(const QHttpServerRequest &)>(
                             [this](const QHttpServerRequest &request) {
...
QVariant TableOfContent::data(const QModelIndex &index, int role) const {
  ...
  if (role == ChapterID) {
    ...
    return mBook->tableOfContent.at(index.row()).toMap().value("chapterName");
  } else if (role == ChapterUrl) {
    return mBook->tableOfContent.at(index.row()).toMap().value("chapterUrl");
  } else if (role == ChapterLevel) {
    return mBook->tableOfContent.at(index.row()).toMap().value("chapterLevel");
  }
...
Q_INVOKABLE QString TableOfContent::markdownToHtml(const QString &markdown,
  ...
  blockHtml = blockHtml.replace(
      imageRegex,
      "<img src=\"\\2" + QString("?%1x%2").arg(w).arg(h) + "\" alt=\"\\1\"/>");
